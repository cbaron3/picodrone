...
int main(void)
{
  ...
  nrf24_cfg radio_cfg;
  memcpy(radio_cfg.address, tx_address , 5);
  radio_cfg.channel = 10;
  radio_cfg.payload_len = 32;

  /* USER CODE BEGIN 2 */
#if defined(TRANSMITTER) && !defined(RECEIVER)
  nrf24_init(&radio_cfg, NRF24_TRANSMITTER);
#endif

#if defined(RECEIVER) && !defined(TRANSMITTER)
  nrf24_init(&radio_cfg, NRF24_RECEIVER);
#endif
  ...
  {
#if defined(TRANSMITTER) && !defined(RECEIVER)
	if(nrf24_transmit(tx_data) == NRF24_ERRC_OK) {
...
#if defined(RECEIVER) && !defined(TRANSMITTER)
	if(nrf24_is_data_available(1) == 1) {
